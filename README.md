...
## Déploiement (pour avoir un lien de démo public)

Streamlit Community Cloud permet d'héberger gratuitement une appli Streamlit directement depuis un repo GitHub.

1. Créer un repo GitHub public et y pousser ce projet.
2. Aller sur [share.streamlit.io](https://share.streamlit.io) et se connecter avec GitHub.
3. Cliquer sur "New app", choisir le repo, la branche, et `app.py` comme fichier principal.
4. Déployer. L'appli est en ligne en quelques minutes avec une URL du type `https://<nom>.streamlit.app`.

Cette URL est celle à mettre sur ton CV ou ton profil LinkedIn.

## Pistes pour aller plus loin

- Étendre l'algorithme CFR au calcul de ranges preflop sur une version simplifiée du vrai Texas Hold'em (heads-up).
- Ajouter un historique multi-mains avec des statistiques de bluff (fréquence réelle vs fréquence théorique).
- Comparer plusieurs variantes de jeux miniatures (Leduc Poker) pour montrer la généralité de l'algorithme.

## Auteur

À compléter avec ton nom et un lien vers ton profil.
